...
class Main:
    ...
    def slippage_report(self):
        ...
        db = pymysql.connect(host="database-1.cc8twgnxgsjl.ap-south-1.rds.amazonaws.com",
                             user="admin", password="FinvantResearch", db="test")
        cur = db.cursor()
        ...
        while True:
            ...
            if now.time() > self.LOGIN_TIME and now.time() < self.SLEEP_TIME and isTradingDay(now):
                try:
                    ...
                    self.config = Config()
                    self.config.refresh_config()
                    self.log_interface = Log_Server_Interface(
                        config=self.config)
                    self.log_interface.postLog(
                        severity='INFO', message='Finvant Alcubierre Mk1 OMS Backend turned on.', publish=1)

                except Exception as e:
                    print(e)
                    Logs.logCritical(
                        severity="CRITICAL", message='Failed to initialise Log server/ADS.', publish=1, tag='OMSB_MAIN_1')
                self.main_loop()
            else:
    ...
    def main_loop(self):
        ...
        self.log_interface.postLog(
            severity='INFO', message='OMS Backend mainloop started.', publish=0)
        ...
        try:
            ...
            print(user_data)
            self.log_interface.postLog(
                severity='INFO', message='User data obtained.', publish=0)
        except Exception as e:
            self.log_interface.postLog(
                severity="CRITICAL", message="Failed to get user data.", publish=1, tag='OMSB_MAIN_6')
        ...
        try:
            ...
            if self.DEBUG == False:
                self.ads_interface.update_config()
            self.log_interface.postLog(
                severity='INFO', message='Spreads obtained.', publish=0)
        except Exception as e:
            self.log_interface.postLog(
                severity="CRITICAL", message="Failed to get spreads.", publish=1, tag='OMSB_MAIN_7')

        try:
            config = Config()
            config.refresh_config()
            self.log_interface.postLog(
                severity='INFO', message='Config file obtained.', publish=0)
        except Exception as e:
            self.log_interface.postLog(
                severity="CRITICAL", message="Failed to get config file.", publish=1, tag='OMSB_MAIN_8')

        try:
            # =================================
            # Starting the LTP gather thread #
            # =================================
            kite_ticker = auto_login_zerodha_ticker(
                config=config, log_interface=self.log_interface, debug=self.DEBUG)
            g = GatherLTP(kws=kite_ticker,
                          log_interface=self.log_interface, config=self.config)
            g.start_LTP_thread()
        except:
            self.log_interface.postLog(
                severity="CRITICAL", message='Failed to start LTP thread.', publish=1, tag='OMSB_MAIN_3')

        try:
            # ===========================
            # Generating user profiles #
            # ===========================
            user_dict = UserProfileGenerator().create_user_profiles(
                user_data=user_data, log_interface=self.log_interface, debug=self.DEBUG)
        except:
            self.log_interface.postLog(
                severity="CRITICAL", message='Failed to generate user dictionary.', publish=1, tag='OMSB_MAIN_4')
        ...
        try:
            ...
            order_handler = OrderHandler(
                user_profiles=user_dict, spread_list=spread_list, log_interface=self.log_interface, debug=self.DEBUG)
            order_handler.start_order_handler()
        except:
            self.log_interface.postLog(
                severity="CRITICAL", message='Failure in order handler.', publish=1, tag='OMSB_MAIN_5')
