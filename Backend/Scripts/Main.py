...
class Main:
    def __init__(self) -> None:
        ...
        self.SLEEP_TIME = datetime.strptime("23:51:00",'%H:%M:%S').time()
        self.start()
        ...
        while True:
            ...
            if now.time() > self.LOGIN_TIME and now.time() < self.SLEEP_TIME and isTradingDay(now):
                try:
                    ...
                    self.log_interface = Log_Server_Interface(config=config)
                    self.log_interface.postLog(severity='INFO',message='Finvant Alcubierre Mk1 OMS Backend turned on.', publish=1)
                    
                except Exception as e:
                    print(e)
                    self.log_interface.postLog(severity="CRITICAL",message='Failed to initialise Log server/ADS.',publish = 1, tag = 'OMSB_MAIN_1')
                self.main_loop()
            else:
                sleep(100)
    ...
    def main_loop(self):
        try:
            ...
            #Create the ADS interface to obtain configs, user data etc
            ...
            self.log_interface.postLog(severity="CRITICAL",message='Failed to get user data/spreads/config.',publish = 1, tag = 'OMSB_MAIN_2')

        try:
            #Get the KWS object to fetch LTP data
