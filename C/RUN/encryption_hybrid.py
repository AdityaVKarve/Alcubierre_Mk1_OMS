...
import datetime
from APIhelper import LOG_Interface
...
class EncryptionHybrid:
    ...
    def __init__(self):
        """ 
        The constructor for EncryptionHybrid class. Initializes the class variables and loads the keys.
        ...
        """
        config = Config()
        self.public_key_path = '../Keys/OMS/public_key_.pem'
        self.private_key_path = '../Keys/OMS/private_key_.pem'
        self.public_key = None
        self.private_key = None
        self.log = LOG_Interface(config.LOG_SERVER_ADDRESS)
        self.load_keys()
    ...
    def generate_keys(self):
        ...
        try:
            ...
            message = {
                "Section": "Test",
                "Severity": "INFO",
                "Message": "Error in generating keys",
                "Send time": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "Publish": 1,
                "Tage" : "OMS_EncryptionHybrid_0"
            }
            self.log.post('post/log', message=str(message))
            return {"Section": SECTION,"Severity": "ERROR","Message": str(e),"Publish": True,"Tags": "OMS_EncryptionHybrid_1"}
    ...
    def load_keys(self):
        ...
        try:
            ...
            message = {
                "Section": "Test",
                "Severity": "INFO",
                "Message": "Error in loading keys : " + str(e),
                "Send time": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "Publish": 1,
                "Tage" : "OMS_EncryptionHybrid_1"
            }
            self.log.post('post/log', message=str(message))
            return {"Section": SECTION,"Severity": "ERROR","Message": str(e),"Publish": True,"Tags": "OMS_EncryptionHybrid_2"}
    ...
    def encrypt(self, message):
        ...
        try:
            ...
            message = {
                "Section": "Test",
                "Severity": "INFO",
                "Message": "Error in encrypting message : " + str(e),
                "Send time": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "Publish": 1,
                "Tage" : "OMS_EncryptionHybrid_2"
            }
            self.log.post('post/log', message=str(message))
            return {"Section": SECTION,"Severity": "ERROR","Message": str(e),"Publish": True,"Tags": "OMS_EncryptionHybrid_3"}
    ...
    def decrypt(self, encrypted_aes_key, encrypted_message):
        ...
        try:
            ...
            message = {
                "Section": "Test",
                "Severity": "INFO",
                "Message": "Error in decrypting message : " + str(e),
                "Send time": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "Publish": 1,
                "Tage" : "OMS_EncryptionHybrid_3"
            }
            self.log.post('post/log', message=str(message))
            traceback.print_exc()
            return {"Section": SECTION,"Severity": "ERROR","Message": str(e),"Publish": True,"Tags": "OMS_EncryptionHybrid_4"}
